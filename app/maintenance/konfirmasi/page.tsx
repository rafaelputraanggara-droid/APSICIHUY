...
export default function KonfirmasiPenyelesaianPage() {
  ...
  const fetchReports = async () => {
    setIsLoading(true);
    try {
      const res = await fetch("/api/maintenance?status=Sedang%20Diperbaiki");
      ...
      <div className="max-w-7xl mx-auto space-y-8 pb-12 animate-fade-in relative">
        <div className="relative overflow-hidden bg-[var(--bg-banner)] p-8 rounded-3xl text-white shadow-xl transition-colors duration-400">
          <div className="relative z-10 space-y-2">
            ...
            <p className="text-[var(--text-muted)] max-w-2xl text-sm sm:text-base font-medium">
              Daftar barang yang sedang dalam proses perbaikan. Konfirmasi penyelesaian dengan mengunggah bukti perbaikan.
            </p>
          </div>
        </div>
        ...
        <div className="bg-[var(--bg-panel)] border border-[var(--border-panel)] rounded-2xl shadow-sm overflow-hidden p-6 transition-colors duration-400">
          {isLoading ? (
            ...
            <p className="text-center py-8 text-[var(--text-muted)]">Tidak ada barang yang sedang diperbaiki.</p>
          ) : (
              ...
              {reports.map((report) => (
                <div key={report.id} className="border border-[var(--border-panel)] rounded-xl p-5 bg-[var(--bg-app)] relative">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <span className="px-3 py-1 bg-[var(--tag-info-bg)] text-[var(--tag-info-text)] text-xs font-bold rounded-full">
                      Sedang Diperbaiki
                    </span>
                  </div>
